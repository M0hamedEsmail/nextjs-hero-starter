...
const validateProjectName = (projectName: string): void => {
  ...
  if (!validNameRegex.test(projectName)) {
    throw new Error(
      'Project name can only contain letters, numbers, hyphens, and underscores.\n' +
        `Invalid characters found in: "${projectName}"`
    );
  }
  ...
  if (reservedNames.includes(projectName.toLowerCase())) {
    throw new Error(
      `"${projectName}" is a reserved name and cannot be used.\n` +
  ...
  if (projectName.length < 1 || projectName.length > 214) {
    throw new Error(
      'Project name must be between 1 and 214 characters.\n' +
        `Current length: ${projectName.length}`
    );
  }

  if (projectName.startsWith('.') || projectName.startsWith('_')) {
    throw new Error('Project name cannot start with a dot (.) or underscore (_)');
  }
};
...
const copyTemplateFiles = async (projectPath: string, options: CliOptions): Promise<void> => {
  ...
  try {
    const templateMap: TemplateMap = {
      basic: 'base-frontend',
      secure: 'base-frontend',
      full: 'base-frontend',
    };

    const templateName = templateMap[template] || 'base-frontend';
  ...
  try {
    ...
    if (error instanceof Error) {
      ...
      if (nodeError.code === 'EACCES') {
        throw new Error(
          'Permission denied. Try running with elevated privileges or check folder permissions.'
        );
      }

      if (nodeError.code === 'ENOSPC') {
        throw new Error('Not enough disk space available. Free up some space and try again.');
      }

      if (nodeError.code === 'ENOENT') {
        throw new Error(
          'Path not found. Make sure the parent directory exists and you have access to it.'
        );
      }
    }
